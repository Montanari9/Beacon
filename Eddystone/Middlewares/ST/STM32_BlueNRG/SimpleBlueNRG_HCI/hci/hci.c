...
void HCI_Process(void)
{
  ...
  if (readPacketListFull) {
    while(BlueNRG_DataPresent()) {
      data_len = Hal_Read_Serial(buffer, HCI_READ_PACKET_SIZE);
      if(data_len > 0)
        HCI_Event_CB(buffer);
    }
...
void HCI_Isr(void)
{
  ...
  while(BlueNRG_DataPresent()){
    if (list_is_empty (&hciReadPktPool) == FALSE){
      ...
      data_len = Hal_Read_Serial(hciReadPacket->dataBuff, HCI_READ_PACKET_SIZE);
      if(data_len > 0){
...
int hci_send_req(struct hci_request *r, BOOL async)
{
  ...
  while(1) {
    ...
    while(1){
      ...
      if(HCI_TIMEOUT != HAL_MAX_DELAY)
      {
        if((HAL_GetTick() - tickstart ) > HCI_TIMEOUT)
        {
          goto failed;
        }
      }
